...
{
    public partial class Proveedor_de_Mantenimiento : Form
    {
        public Proveedor_de_Mantenimiento()
        {
            InitializeComponent();
        }
        ...
        private void Proveedor_de_Mantenimiento_Load(object sender, EventArgs e)
        {
            actualizar();
        }
        ...
        public void nivel()
        {
            ...
            OdbcDataAdapter da = new OdbcDataAdapter("SELECT * FROM proveedor_mantenimiento where estado <> 'INACTIVO'", con);
            da.Fill(dt);

            foreach (DataRow dr in dt.Rows)
            {
                TreeNode parent = new TreeNode(dr["nombre_prov_mant"].ToString());
                string value = dr["id_prov_mante_pk"].ToString();
                parent.Tag = dr["id_prov_mante_pk"].ToString();
        ...
        private void btn_guardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string tabla = "proveedor_mantenimiento";
                    if (Editar)
                    {
        ...
        public void modificar()
        {
            try
            {
                Editar = true;
                fn.ActivarControles(groupBox1);
                atributo = "id_prov_mante_pk";
                Codigo = txt_id_copia.Text;
                txt_prov_man.Text = txt_copia.Text;
                // actualizar();
                ...
                if (resultado == DialogResult.Yes)
                {
                    ...
                    foreach (TreeNode n in treeView1.Nodes)
                    {
                        ...
                        if (n.Checked == true)
                        {
                            ...
                            String atributo2 = "id_prov_mante_pk ";
