...
namespace WindowsFormsApp1
{
    public partial class Modelo : Form
    {
        ...
        public void nivel()
        {
            ...
            OdbcDataAdapter da = new OdbcDataAdapter("SELECT * FROM modelo where estado <> 'INACTIVO'", con);
            da.Fill(dt);

            foreach (DataRow dr in dt.Rows)
            {
                TreeNode parent = new TreeNode(dr["nombre_modelo"].ToString());
                string value = dr["id_modelo_pk"].ToString();
                parent.Tag = dr["id_modelo_pk"].ToString();
        ...
        public void modificar()
        {
            try
            {
                ...
                txt_modelo.Text = txt_copia.Text;
                // actualizar();
