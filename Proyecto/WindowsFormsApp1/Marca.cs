...
namespace WindowsFormsApp1
{
    public partial class Marca : Form
    {
        ...
        public void nivel()
        {
            ...
            OdbcDataAdapter da = new OdbcDataAdapter("SELECT * FROM marca where estado <> 'INACTIVO'", con);
            da.Fill(dt);

            foreach (DataRow dr in dt.Rows)
            {
                TreeNode parent = new TreeNode(dr["nombre_marca"].ToString());
                string value = dr["id_marca_pk"].ToString();
                parent.Tag = dr["id_marca_pk"].ToString();
        ...
        public void modificar()
        {
            try
            {
                Editar = true;
                fn.ActivarControles(groupBox1);
                atributo = "id_marca_pk";
                Codigo = txt_id_copia.Text;
                txt_marca.Text = txt_copia.Text;
                // actualizar();
