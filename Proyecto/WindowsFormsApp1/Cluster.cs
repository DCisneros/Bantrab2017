...
namespace WindowsFormsApp1
{
    public partial class cluster : Form
    {
        ...
        public void nivel()
        {
            ...
            OdbcDataAdapter da = new OdbcDataAdapter("SELECT * FROM cluster where estado <> 'INACTIVO'", con);
            da.Fill(dt);

            foreach (DataRow dr in dt.Rows)
            {
                TreeNode parent = new TreeNode(dr["nombre_cluster"].ToString());
                string value = dr["id_cluster_pk"].ToString();
                parent.Tag = dr["id_cluster_pk"].ToString();
        ...
        public void modificar()
        {
            try
            {
                Editar = true;
                fn.ActivarControles(groupBox1);
                atributo = "id_cluster_pk";
                Codigo = txt_id_copia.Text;
                txt_cluster.Text = txt_copia.Text;
                // actualizar();
                ...
                if (resultado == DialogResult.Yes)
                {
                    ...
                    foreach (TreeNode n in treeView1.Nodes)
                    {
                        ...
                        if (n.Checked == true)
                        {
                            ...
                            String atributo2 = "id_cluster_pk ";
